...
下图从 Retail 数据集最终保留的 Epoch 90 检查点中提取。Epoch 从 0 开始编号。

![学习到的行为转移衰减热力图](figures/transition_decay_epoch90/transition_decay_heatmaps.png)

验证日志在 Epoch 78 达到最佳 NDCG@10（0.728079），但该检查点权重没有保留；最终保存的 Epoch 90 检查点对应 NDCG@10 为 0.725703。因此，该图用于解释最终保留模型的参数，并不声称展示的是最佳验证指标检查点。图中系数在约 0.003 至 2.082 之间，不同行为转移和注意力头存在明显差异。例如，“历史浏览 `pv` → 当前购买 `buy`”在头 1 中的系数为 2.082，在头 2 中仅为 0.005，体现出快衰减与慢衰减的互补模式。热力图解释的是全局学习参数，不能作为行为之间存在因果关系的证据。两个注意力头的矩阵、头间平均矩阵和检查点元数据均保存在同一图片目录中。

## Retail 预研结果（两个随机种子）

最终 Retail 预研实验采用两个随机种子（`42` 和 `2024`）。所有模型使用相同的数据划分和评测协议；每个随机种子下均选择验证集 NDCG@10 最佳的检查点，下表报告两个随机种子结果的算术平均值。

| 模型 | 平均 NDCG@10 | 相对 GEAR 的绝对变化 | 相对 GEAR 的相对变化 |
|---|---:|---:|---:|
| GEAR | 0.71665 | - | - |
...
GEAR-T 相比 GEAR 的平均提升几乎为零，因此两个随机种子的结果暂时不能证明仅将原始注意力头级斜率设为可学习参数能够带来稳定收益。BT-GEAR-S 与 BT-GEAR 的平均结果均稳定高于 GEAR，支持根据行为转移类型调整时间衰减的有效性。BT-GEAR 与 BT-GEAR-S 之间仅相差 `0.00018`，因此每个注意力头使用独立转移矩阵的额外收益目前尚不明确。现有结果支持完整的行为转移感知时间衰减方案，但不足以声称每个独立组件都分别带来了稳定贡献。形成论文级结论前仍需补充更多随机种子、更多数据集和统计显著性检验。
